...
Collision_detector::Collision_detector(Player &player, std::vector<std::unique_ptr<Terrain> > &loaded_terrain,
                                       ...
                                       EnemyController &enemy_controller, Map_parser &map_parser,
                                       SaverAndLoader &saveManager): player(player), saveManager(saveManager),
                                                                     hatch(nullptr),
                                                                     reward(Weapon(std::ref(projectile_manager))),
                                                                     enemy_controller(enemy_controller),
                                                                     loaded_terrain(loaded_terrain),
                                                                     projectile_manager(projectile_manager),
                                                                     map_parser(map_parser), rewardPickedUp(false) {
}
...
void Collision_detector::checkColisionWithPlayer() {
    for (auto &t: loaded_terrain) {
        ...
        if (player.getSprite().getGlobalBounds().findIntersection(t->getSprite().getGlobalBounds()) && (*t).
            hasCollision) {
